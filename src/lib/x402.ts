...
export function createQuote(input: Omit<QuoteRecord, 'quoteId' | 'status' | 'expiresAt'>) {
...
export function markQuotePaid(quoteId: string, txHash: `0x${string}`, payer: `0x${string}`) {
  const store = getStore()
  const quote = store.quotes.get(quoteId)
  if (!quote) return undefined
